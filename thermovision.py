...
CONFIG = {**image_config,
          ...
          'hough_threshold': 40,  # Threshold for Hough transform (line detection)
          'hough_line_length': 50,  # Minimum line length detected by Hough
          'hough_line_gap': 10,  # Maximum gap between segments to consider as a single line
          "angles": [-10, 90],  # Reference angles for the thermometer
          "temperatures": [20, 80]  # Temperatures corresponding to reference angles
          }
...
def display_line(line, ax):
    ...
    mid_point = [(line[0][0] + line[1][0]) / 2 + 30, (line[0][1] + line[1][1]) / 2 - 25]
...
def main():
    ...
    # Get the ROIs
    roi_1_coords = get_roi_from_center(center)
    ...
    for line in lines:
        ...
        # uncomment to debug line detection
        # else:
